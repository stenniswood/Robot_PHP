...
function onDocumentKeyDown(event) {
	...
	var str = "Box[0] position = < "+object_meshes[0].position.x +","+ object_meshes[0].position.y +","+ object_meshes[0].position.z +">";		
	var zero_vec = new THREE.Vector3(0,0,0);

	var unitX = new THREE.Vector3( 1,0,0 );
	var unitY = new THREE.Vector3( 0,1,0 );
	var unitZ = new THREE.Vector3( 0,0,1 );	
	rotation_matrix_only( 0, unitX );
	rotation_matrix_only( 0, unitY );
	rotation_matrix_only( 0, unitZ );

	str += "Box[1] position = < "+object_meshes[1].position.x +","+ object_meshes[1].position.y +","+ object_meshes[1].position.z +"><br>";		
	info.innerHTML = str;
		...
		} else if (key == ',') {		object_meshes[0].rotation.x += 0.25;
		} else if (key == '<') {		object_meshes[0].rotation.x -= 0.25;
